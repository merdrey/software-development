...
Java_com_example_lab1_MainActivity_transaction(JNIEnv *xenv, jobject xthiz, jbyteArray xtrd) {
    ...
    auto trd = (jbyteArray) xenv->NewGlobalRef(xtrd);
    ...
    std::thread t([thiz, trd] {
        ...
        auto* p = (uint8_t*)env->GetByteArrayElements (trd, 0);
        ...
        buf[12] = 0x00;
        auto jamount = (jstring) env->NewStringUTF(buf);
        int ptc = 3;
        while (ptc > 0) {

            auto pin = (jstring) env->CallObjectMethod(thiz, id, ptc, jamount);
